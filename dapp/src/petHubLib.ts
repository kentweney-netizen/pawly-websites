...
/**
 * PAWLY Pet Hub v0.2.33 — dual-sign like Payment: userPartialSign + sponsor-dapp-tx.
 * Fee payer always hot wallet BPFiVa5. No user-SOL fallback. USDC/USDT/SOL swap then till.
...
async function sendHubSwapTx(opts: { conn: Connection; tx: VersionedTransaction; sendTransaction?: HubSend; signTransaction?: HubSign; wallet?: HubWallet }): Promise<string> {
...
async function swapCoinToTillPawly(opts: { from: PublicKey; coin: PayCoin; coinAmount: number; conn: Connection; sendTransaction?: HubSend; signTransaction?: HubSign; wallet?: HubWallet; pawlyList?: number }): Promise<string> {
  ...
  for (let attempt = 0; attempt < slips.length; attempt++) {
    try {
      ...
      swapSig = await sendHubSwapTx({ conn: opts.conn, tx, sendTransaction: opts.sendTransaction, signTransaction: opts.signTransaction, wallet: opts.wallet });
...
export async function payHub(opts: { from: PublicKey; coin: PayCoin; amount: number; signTransaction?: HubSign; sendTransaction?: HubSend; wallet?: HubWallet }): Promise<string> {
  ...
  if (opts.coin !== "PAWLY") {
    return await swapCoinToTillPawly({ from: opts.from, coin: opts.coin, coinAmount: opts.amount, conn, sendTransaction: opts.sendTransaction, signTransaction: opts.signTransaction, wallet: opts.wallet, pawlyList: 0 });
  }
