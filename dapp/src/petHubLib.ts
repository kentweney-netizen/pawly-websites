...
/**
 * PAWLY Pet Hub v0.2.34 — dual-sign like Payment: userPartialSign + sponsor-dapp-tx.
 * Fee payer always hot wallet BPFiVa5. No user-SOL fallback.
 * USDC/USDT/SOL one-shot: swap + PAWLY to till in the SAME sponsored tx.
...
async function userPartialSign(tx: VersionedTransaction, wallet?: HubWallet | null, signTransaction?: HubSign): Promise<VersionedTransaction> {
  ...
  let last: unknown = "Wallet cannot partial-sign / wallet needs signTransaction for sponsor pay";
...
async function buildHubSwapTx(opts: { inputMint: string; amount: string; user: string; inputAccount?: string; outputAccount?: string; slippageBps: number }) {
    ...
    try {
      const r = await fetch(path, {
        ...
        body: JSON.stringify({
          ...
          outputAccount: opts.outputAccount || "",
          destinationTokenAccount: opts.outputAccount || "",
          slippageBps: opts.slippageBps,
        }),
      });
      const d = (await r.json()) as { swapTransaction?: string; error?: string; outAmount?: string; minOut?: string; destMode?: string; venue?: string };
...
async function sendHubOneShot(opts: {
  conn: Connection;
  ...
  const ixs = TransactionMessage.decompile(opts.tx.message, { addressLookupTableAccounts: alts }).instructions.slice();
  if (opts.destMode !== "till" && opts.minOutRaw > 0) {
    const userAta = await getAssociatedTokenAddress(mint, opts.from, false, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
    ixs.push(createAssociatedTokenAccountIdempotentInstruction(sponsor, opts.tillAta, till, mint, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID));
    ixs.push(createTransferCheckedInstruction(userAta, mint, opts.tillAta, opts.from, opts.minOutRaw, 6, [], TOKEN_PROGRAM_ID));
  }
...
async function swapCoinToTillPawly(opts: { from: PublicKey; coin: PayCoin; coinAmount: number; conn: Connection; sendTransaction?: HubSend; signTransaction?: HubSign; wallet?: HubWallet; pawlyList?: number }): Promise<string> {
  ...
  const tillBefore = await pawlyUiOf(opts.conn, till);
  ...
  for (let attempt = 0; attempt < slips.length; attempt++) {
    try {
      const pack = await buildHubSwapTx({
        ...
        inputAccount,
        outputAccount: tillAta.toBase58(),
        slippageBps: slips[attempt],
      });
      const tx = VersionedTransaction.deserialize(b64ToBytes(String(pack.swapTransaction)));
      const minOutRaw = Math.max(0, Math.floor(Number(pack.minOut || pack.outAmount || 0)));
      const destMode = String(pack.destMode || "") === "till" ? "till" : "user";
      const sig = await sendHubOneShot({
        conn: opts.conn,
        tx,
      ...
      let tillAfter = tillBefore;
      for (let i = 0; i < 10; i++) {
        tillAfter = await pawlyUiOf(opts.conn, till);
        if (tillAfter > tillBefore + 0.000001) break;
        await sleepHub(400);
      }
      if (tillAfter <= tillBefore + 0.000001) throw new Error("Swap ok but till did not receive PAWLY");
      return sig;
    } catch (e) {
      if (isUserCancel(e)) throw e;
      lastErr = e;
  ...
  throw lastErr instanceof Error ? lastErr : new Error(String(lastErr || "One-shot till pay failed"));
}
