...
import type { HubSign, HubSend, HubWallet, PayCoin, PayPhase, PayPhaseFn } from "./petHubSend";
...
async function postSponsor(signed: VersionedTransaction) {
  ...
  }, 12000, "Sponsor timeout 12s / 代付超时");
...
async function userSign(tx: VersionedTransaction, wallet?: HubWallet | null, signTransaction?: HubSign) {
  const fns: HubSign[] = [];
  if (typeof signTransaction === "function") fns.push(signTransaction);
  const ad = wallet && (wallet.adapter || (wallet.wallet && wallet.wallet.adapter));
  if (ad && typeof ad.signTransaction === "function") fns.push(ad.signTransaction);
  if (wallet && typeof wallet.signTransaction === "function") fns.push(wallet.signTransaction);
  let last: unknown = "Wallet cannot partial-sign";
  for (let i = 0; i < fns.length; i++) {
    try {
      const out = await fns[i](tx);
      if (out && typeof (out as VersionedTransaction).serialize === "function") return out;
      return tx;
    } catch (e) { if (isCancel(e)) throw e; last = e; }
  }
...
async function wrapSol(from: PublicKey, lamports: number, c: Connection, wallet?: HubWallet | null, signTransaction?: HubSign) {
  const sponsor = new PublicKey(SHOP_TILL);
  const mint = new PublicKey(WSOL_MINT);
  const wsolAta = await getAssociatedTokenAddress(mint, from, false, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
  const ixs = [
    createAssociatedTokenAccountIdempotentInstruction(sponsor, wsolAta, from, mint, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID),
    SystemProgram.transfer({ fromPubkey: from, toPubkey: wsolAta, lamports: Math.max(1, Math.round(lamports)) }),
  ...
  const signed = await userSign(vtx, wallet, signTransaction);
  await postSponsor(signed);
  return wsolAta.toBase58();
}
async function buildSwap(opts: { inputMint: string; amount: string; user: string; inputAccount?: string; slippageBps: number; wrapSol?: boolean }) {
...
async function sendSwap(c: Connection, tx: VersionedTransaction, wallet?: HubWallet | null, signTransaction?: HubSign) {
  const sponsor = new PublicKey(SHOP_TILL);
  ...
  const ixs = TransactionMessage.decompile(tx.message, { addressLookupTableAccounts: alts }).instructions;
  const { blockhash } = await c.getLatestBlockhash();
  ...
  const signed = await userSign(vtx, wallet, signTransaction);
  return await postSponsor(signed);
}

export async function swapCoinToTill(opts: {
  from: PublicKey; coin: PayCoin; coinAmount: number;
  signTransaction?: HubSign; sendTransaction?: HubSend; wallet?: HubWallet | null;
  pawlyList?: number; onPhase?: PayPhaseFn;
}): Promise<string> {
  ...
  if (isSol) {
    ...
    inputAccount = await wrapSol(opts.from, rawIn, c, opts.wallet, opts.signTransaction);
  } else {
  ...
  for (let i = 0; i < slips.length; i++) {
    try {
      ...
      const pack = await buildSwap({ inputMint, amount: String(rawIn), user: opts.from.toBase58(), inputAccount, slippageBps: slips[i], wrapSol: false });
      const tx = VersionedTransaction.deserialize(b64ToBytes(String(pack.swapTransaction)));
      say("sign", "Sign swap in wallet");
      swapSig = await sendSwap(c, tx, opts.wallet, opts.signTransaction);
  ...
  if (!(payAmt > 0)) throw new Error("Swap " + swapSig.slice(0, 8) + " but PAWLY not in wallet yet / 已兑换请稍候再付店柜");
  ...
  throw hop instanceof Error ? hop : new Error(String(hop || "Till transfer failed"));
}
