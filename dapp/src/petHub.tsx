...
export function PetHubPage() {
  ...
  const wallet = usePawlyWallet() as { publicKey?: PublicKey | null; signTransaction?: (tx: VersionedTransaction) => Promise<VersionedTransaction>; sendTransaction?: (tx: VersionedTransaction, conn: Connection) => Promise<string> };
  ...
  const confirmPay = async () => {
    ...
    setBusy(true); setNote(payCoin === "PAWLY" ? "Paying PAWLY to till..." : "Swapping " + payCoin + " to PAWLY, then till...");
    try {
      const sig = await payHub({ from: wallet.publicKey, coin: payCoin, amount: payAmt.amount, signTransaction: wallet.signTransaction, sendTransaction: wallet.sendTransaction });
      ...
      <div style={{ flex: 1, minHeight: 0, position: "relative", background: "#0a1016" }}>
        ...
        {scene === "street" && walkers.map((p, i) => (
          <button key={p.id} type="button" className={"pawly-stroll pawly-stroll-" + (i % 4)} onClick={() => { setFocusId(p.id); setFeedWarn(true); }}>
            <PetRig pet={{ species: p.species, level: Math.max(1, Number(p.level || 1)), emoji: p.emoji, name: p.name }} size={52} moving />
          </button>
        ))}
      </div>
