/**
 * PAWLY Pet Hub v0.2.26 LOCK — 3-layer GameFi:
 * shared /pet-hub-street-walk.mp4 + this wallet Lv1+ pixel stroll (max 4) + card heads.
 * No fused 3D heads. Rescue stays emoji.
 */
...
function loadPets(w: string): PetRec[] {
  ...
  try {
    ...
    return Array.isArray(list) ? (list as PetRec[]).filter((p) => p && p.id) : [];
  } catch {
    return [];
  }
}
  ...
  return (
    <div style={{ height: "100dvh", overflow: "hidden", background: "#070b10", color: "#e8eef7", display: "flex", flexDirection: "column", position: "relative", maxWidth: 430, margin: "0 auto" }}>
      ...
      <div style={{ padding: "calc(env(safe-area-inset-top, 16px) + 18px) 10px 8px" }}>
        <div style={{ color: "#00ff9d", fontWeight: 800 }}>{TITLE[scene]} · v0.2.26</div>
      ...
      <div style={{ flex: 1, minHeight: 0, position: "relative", background: "#0a1016" }}>
        ...
        {scene === "street" && walkers.map((p, i) => (
          <button key={p.id} type="button" className={"pawly-stroll pawly-stroll-" + (i % 4)} onClick={() => { setFocusId(p.id); setFeedWarn(true); }}>
            <PetRig pet={{ species: p.species, level: Math.max(1, Number(p.level || 1)), emoji: p.emoji, name: p.name }} size={72} moving />
          </button>
        ))}
      </div>
      <div style={{ zIndex: 2, padding: "8px 8px 10px", background: "#070b10", maxHeight: "46dvh", overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center", padding: "6px 0 10px" }}>
            {pets.length ? pets.map((p) => (
              <button key={p.id} type="button" onClick={() => { setFocusId(p.id); setFeedWarn(true); }} style={{ background: focusId === p.id ? "rgba(0,255,157,0.18)" : "transparent", border: "none", color: "#e8eef7" }}>
                <PetRig pet={{ species: p.species, level: Number(p.level || 0), emoji: p.emoji, name: p.name }} size={88} />
          ...
          <div>
            ...
            {COMPANIONS.map((c) => (
              <button key={c.species} type="button" style={rowBtn} onClick={() => openCart({ title: "Adopt " + c.label, amount: c.pricePawly, kind: "adopt", species: c.species, emoji: c.emoji })}>
                <span style={{ display: "flex", alignItems: "center", gap: 8 }}><PetRig pet={{ species: c.species, level: 1, emoji: c.emoji, name: c.label }} size={36} />{c.label}</span>
        ...
        {(scene === "hotel" || scene === "groom") && (
          <button type="button" style={{ ...primary, width: "100%" }} onClick={() => openCart({ title: TITLE[scene], amount: scene === "hotel" ? 50 : 30, kind: "service" })}>
            Pay {scene === "hotel" ? 50 : 30} PAWLY
          </button>
        )}
        {note ? <div style={{ color: "#ffb4b4", fontSize: 11, marginTop: 6, wordBreak: "break-word" }}>{note}</div> : null}
        {lastSig ? <div style={{ color: "#9f8", fontSize: 10, marginTop: 6, wordBreak: "break-all" }}>{lastSig}</div> : null}
      ...
      {feedWarn ? (
        <div style={{ position: "absolute", inset: 0, zIndex: 5, background: "rgba(0,0,0,0.62)", display: "flex", alignItems: "flex-end" }} onClick={() => setFeedWarn(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: "#101820", borderRadius: "16px 16px 0 0", padding: 16 }}>
            ...
            <div style={{ fontSize: 13, margin: "8px 0 12px" }}>Max 3 feeds per day. 10 feeds = Lv1 body on the street.</div>
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: "#101820", borderRadius: "16px 16px 0 0", padding: 16 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", margin: "8px 0" }}>
              {(["PAWLY", "USDC", "USDT", "SOL"] as PayCoin[]).map((c) => (
                <button key={c} type="button" onClick={() => setPayCoin(c)} style={{ ...ghost, borderColor: payCoin === c ? "#00ff9d" : "rgba(255,255,255,0.2)", color: payCoin === c ? "#00ff9d" : "#c8ffe8" }}>{c}</button>
              ))}
            </div>
