...
if "already fed" not in t[t.find("const openCart"): t.find("const openCart")+400]:
    t = t.replace(
...
if "cart.kind === \"food\" || cart.kind === \"feed\"" not in t[t.find("const confirmPay"):t.find("const confirmPay")+700]:
    t = t.replace(
...
t = t.replace(
    ...
    'onClick={() => { const pet = pickFeedPet(pets, focusId); const n = feedsTodayOf(pet); if (!pet) { setFeedWarn(false); setNote("Adopt a pet first / 先领养"); return; } if (n >= FEED_DAY_MAX) { setFeedWarn(false); setNote(pet.name + " already fed " + n + "/" + FEED_DAY_MAX + " today. Come back tomorrow / 今日已喂满 3 次"); return; } setFeedWarn(false); setScene("shop"); setShopView("food"); }}',
    1,
)
t = t.replace(
...
p.write_text(t)
print("ok", "v0.10.3" in t, "FEED_DAY_MAX" in t, "already fed" in t)
